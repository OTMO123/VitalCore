...
function Log-Fix {
    ...
    Write-Host "$icon [$Category] $Message" -ForegroundColor $(
        switch ($Status) {
            "SUCCESS" { "Green" }
            "WARNING" { "Yellow" }
            "ERROR" { "Red" }
...
function Install-DependenciesSafely {
    ...
    try {
        ...
        $result = & $PythonCommand @installArgs
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            foreach ($package in $keyPackages) {
                try {
                    $testResult = & $PythonCommand -c "import $($package.replace('-', '_')); print('OK')" 2>&1
            ...
            Log-Fix "Dependencies" "SUCCESS" "Verified packages: $($verifiedPackages.Count)/$($keyPackages.Count)" @{
                "verified" = ($verifiedPackages -join ", ")
            }
            return $true
        }
        else {
...
"@
        ...
        $testScript | Out-File "temp_config_test.py" -Encoding UTF8
...
try {
    & $PythonCommand -c "import fastapi, pydantic, sqlalchemy" 2>`$null
    if (`$LASTEXITCODE -eq 0) {
...
try {
    & $PythonCommand -c "from app.core.config import get_settings; get_settings()" 2>`$null
    if (`$LASTEXITCODE -eq 0) {
...
"@
    ...
    try {
        $startupScript | Out-File "start_safe.ps1" -Encoding UTF8
        Log-Fix "Startup" "SUCCESS" "Safe startup script created"
        Log-Fix "Startup" "INFO" "Run with: .\start_safe.ps1" @{
