...
try {
    $testCount = python -m pytest --collect-only -q 2>$null | Select-String "test_" | Measure-Object | Select-Object -ExpandProperty Count
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "   ✅ AI/ML and FHIR TESTS: PASSED" -ForegroundColor Green
        $testResults += "AI/ML and FHIR Tests: PASSED"
    } else {
        Write-Host "   ❌ AI/ML and FHIR TESTS: FAILED" -ForegroundColor Red  
        $testResults += "AI/ML and FHIR Tests: FAILED"
    }
} catch {
    Write-Host "   ❌ AI/ML and FHIR TESTS: ERROR" -ForegroundColor Red
    $testResults += "AI/ML and FHIR Tests: ERROR"
}
...
Write-Host "`n🌐 PHASE 10: Quick Test Sample" -ForegroundColor Cyan
Write-Host "===============================" -ForegroundColor Cyan

try {
    Write-Host "Running quick test sample for validation..." -ForegroundColor Yellow
    python -m pytest app/tests/smoke/test_basic.py -v
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "   ✅ SAMPLE TESTS: PASSED" -ForegroundColor Green
        $testResults += "Sample Tests: PASSED"
    } else {
        Write-Host "   ❌ SAMPLE TESTS: FAILED" -ForegroundColor Yellow
        $testResults += "Sample Tests: PARTIAL"
    }
} catch {
    Write-Host "   ❌ SAMPLE TESTS: ERROR" -ForegroundColor Red
    $testResults += "Sample Tests: ERROR"
}
