...
from app.modules.healthcare_records.fhir_rest_api import (
    ...
    public_router as fhir_public_router,
)
...
try:
    ...
    if "marshmallow" in error_msg and "__version_info__" in error_msg:
        print(
            f"Vector store disabled: marshmallow version conflict detected - {str(e)}"
        )
    ...
    try:
        ...
        for key_name, key_value in required_keys.items():
            if not key_value:
                ...
                short_keys.append(
                    f"{key_name} (length: {len(key_value)}, required: 32+)"
                )
        ...
        if decrypted != test_data:
            raise RuntimeError(
                "Encryption service validation failed: decryption mismatch"
            )
        ...
        from app.modules.audit_logger.event_handlers import (
            register_soc2_event_handlers,
            register_simple_event_bridge,
        )
...
def create_app() -> FastAPI:
    ...
    app.add_exception_handler(
        InvalidWorkflowStatusError, workflow_transition_error_handler
    )
    app.add_exception_handler(
        ProviderAuthorizationError, insufficient_permissions_handler
    )
    ...
    app.include_router(
        security_router, prefix="/api/v1/security", tags=["Security Monitoring"]
    )
    ...
    async def clinical_documents_alias():
        ...
        return {
            "message": "Clinical documents available at /api/v1/healthcare/documents",
            "endpoints": [
    ...
    async def health_check():
        ...
        try:
            ...
            health_data["system_monitoring"] = (
                "psutil not available - install for full monitoring"
            )
        except Exception as e:
        ...
        try:
            if hasattr(app.state, "event_bus") and app.state.event_bus:
                ...
                health_data["event_bus"] = {
                    "status": (
                        "operational" if event_bus.hybrid_bus.running else "stopped"
                    ),
