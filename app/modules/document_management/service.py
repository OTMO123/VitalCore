...
from app.core.audit_logger import AuditEventType, AuditSeverity, audit_logger
...
class DocumentStorageService:
    ...
    async def _create_audit_record(
        ...
        try:
            ...
            prev_audit_query = (
                select(DocumentAccessAudit)
                .where(DocumentAccessAudit.document_id == document_id)
                .order_by(desc(DocumentAccessAudit.block_number))
                .limit(1)
            )
    ...
    async def _verify_patient_access(
        self, db: AsyncSession, patient_id: str, user_id: str
    ) -> bool:
        ...
        try:
            ...
            patient_query = select(Patient).where(
                and_(Patient.id == patient_id, Patient.soft_deleted_at.is_(None))
            )
    ...
    async def download_document(
        self, db: AsyncSession, document_id: str, context: AccessContext
        ...
        async def _download_operation():
            try:
                ...
                file_data = await self.storage_backend.retrieve_document(
                    storage_key=document.storage_path, bucket=document.storage_bucket
                )
    ...
    async def search_documents(
        ...
        try:
            ...
            if search_request.patient_id:
                ...
                query = query.where(
                    DocumentStorage.patient_id == search_request.patient_id
                )
            ...
            if search_request.document_types:
                query = query.where(
                    DocumentStorage.document_type.in_(search_request.document_types)
                )
                count_query = count_query.where(
                    DocumentStorage.document_type.in_(search_request.document_types)
                )
            ...
            if search_request.document_category:
                query = query.where(
                    DocumentStorage.document_category
                    == search_request.document_category
                )
                count_query = count_query.where(
                    DocumentStorage.document_category
                    == search_request.document_category
            ...
            if search_request.tags:
                ...
                count_query = count_query.where(
                    DocumentStorage.tags.overlap(search_request.tags)
                )

            if search_request.search_text:
                # Full-text search in extracted text
                query = query.where(
                    DocumentStorage.extracted_text.ilike(
                        f"%{search_request.search_text}%"
                    )
                )
                count_query = count_query.where(
                    DocumentStorage.extracted_text.ilike(
            ...
            if search_request.date_from:
                query = query.where(
                    DocumentStorage.uploaded_at >= search_request.date_from
                )
                count_query = count_query.where(
                    DocumentStorage.uploaded_at >= search_request.date_from
                )
            ...
            if search_request.date_to:
                query = query.where(
                    DocumentStorage.uploaded_at <= search_request.date_to
                )
                count_query = count_query.where(
                    DocumentStorage.uploaded_at <= search_request.date_to
                )
            ...
            for doc in documents:
                doc_response = DocumentMetadataResponse(
                    ...
                    version=doc.version,
                    parent_document_id=(
                        str(doc.parent_document_id) if doc.parent_document_id else None
                    ),
            ...
            self.logger.error(
                "Document search failed", error=str(e), user_id=context.user_id
            )
            raise
    ...
    async def get_document_metadata(
        ...
        try:
            ...
            stmt = select(DocumentStorage).where(
                DocumentStorage.document_id == document_id
            )
            ...
            await self._log_phi_access(
                db=db, document_id=document_id, user_id=context.user_id, context=context
            )
    ...
    async def get_document_statistics(
        ...
        try:
            ...
            # Get total count
            count_query = select(func.count(DocumentStorage.document_id)).select_from(
                base_query.subquery()
            )
            ...
            type_query = (
                ...
                .group_by(DocumentStorage.document_type)
            )

            type_result = await db.execute(type_query)
            documents_by_type = {
                row.document_type.value: row.count for row in type_result.fetchall()
            }

            # Get recent uploads (last 10)
            recent_query = base_query.order_by(desc(DocumentStorage.uploaded_at)).limit(
                10
            )
            ...
            storage_query = select(func.sum(DocumentStorage.file_size)).select_from(
                base_query.subquery()
            )
    ...
    async def bulk_delete_documents(
        ...
        try:
            ...
            self.logger.error(
                "Bulk deletion failed", user_id=context.user_id, error=str(e)
            )
            raise
    ...
    async def bulk_update_tags(
        ...
        try:
            ...
            await db.rollback()
            self.logger.error(
                "Bulk tag update failed", user_id=context.user_id, error=str(e)
            )
            raise


# Factory function to create service instance
async def get_document_service(storage_backend=None) -> DocumentStorageService:
