...
class SecureStorageService:
    ...
    async def secure_retrieve(
        ...
        try:
            ...
            metadata = await self.storage_backend.get_document_metadata(
                storage_key, bucket
            )

            # Verify user has access to this patient's documents
            patient_id = metadata.get("patient-id")
            if patient_id and not await self._verify_patient_access(
    ...
    async def secure_delete(
        self, storage_key: str, bucket: str, context: AccessContext, reason: str
    ) -> bool:
        """
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    "Document not found for deletion", storage_key=storage_key
                )
                return False
            ...
            if success:
                ...
                await audit_logger.log_event(
                    ...
                    severity=AuditSeverity.MEDIUM,  # Deletions are notable events
                )
    ...
    async def get_storage_statistics(self, bucket: str = "documents") -> Dict[str, Any]:
        """
...
def get_secure_storage_service(storage_backend=None) -> SecureStorageService:
