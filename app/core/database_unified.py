...
class RolePermission(BaseModel):
    ...
    permission_id: Mapped[uuid.UUID] = mapped_column(
        UUIDType(), ForeignKey("permissions.id")
    )
...
class AuditLog(BaseModel):
    ...
    soc2_category: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )  # String to support enum values

    # User and session context
    user_id: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, index=True
    )  # Changed to string to support system users
    ...
    headers: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True
    )  # Added headers field for SOC2 tests
    config_metadata: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
    compliance_tags: Mapped[Optional[List[str]]] = mapped_column(
    ...
    log_hash: Mapped[Optional[str]] = mapped_column(
...
class Patient(BaseModel, SoftDeleteMixin):
    ...
    iris_last_sync_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True
    )

    # Clinical workflows relationship
    clinical_workflows: Mapped[List["ClinicalWorkflow"]] = relationship(
        "ClinicalWorkflow", back_populates="patient", cascade="all, delete-orphan"
    )
    ...
    immunizations: Mapped[
        List["app.modules.healthcare_records.models.Immunization"]
    ] = relationship(
...
class ClinicalDocument(BaseModel, SoftDeleteMixin):
    ...
    # FHIR metadata
    fhir_resource_type: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True
    )
...
class Consent(BaseModel):
    ...
    effective_period_start: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False
    )
    effective_period_end: Mapped[Optional[datetime]] = mapped_column(
    ...
    policy_references: Mapped[Optional[List[str]]] = mapped_column(
        ArrayType(String), nullable=True
    )
    ...
    guardian_relationship: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True
    )
    ...
    superseded_by: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUIDType(), ForeignKey("consents.id"), nullable=True
    )
...
class PHIAccessLog(BaseModel):
    ...
    access_session_id: Mapped[str] = mapped_column(
    ...
    clinical_document_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUIDType(), ForeignKey("clinical_documents.id"), nullable=True
    )
    ...
    phi_fields_accessed: Mapped[List[str]] = mapped_column(
        ArrayType(String), nullable=False
    )
    ...
    data_classification: Mapped[DataClassification] = mapped_column(
        Enum(DataClassification, name="dataclassification"),
        default=DataClassification.PHI.value,
    )
    ...
    # Timing
    access_started_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False
    )
    access_ended_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    ...
    supervisor: Mapped[Optional["User"]] = relationship(
        "User", foreign_keys=[supervisor_approval]
    )

    __table_args__ = (
        CheckConstraint(
            "access_type IN ('view', 'edit', 'print', 'export', 'delete', 'bulk_access', 'create')",
...
class DocumentStorage(BaseModel):
    ...
    tags: Mapped[Optional[List[str]]] = mapped_column(
        ArrayType(String), nullable=True, default=[]
    )

    # Versioning
    version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    parent_document_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUIDType(), ForeignKey("document_storage.id"), nullable=True
    )
    ...
    access_log_ids: Mapped[Optional[List[uuid.UUID]]] = mapped_column(
        ArrayType(UUIDType), nullable=True, default=[]
    )
    compliance_metadata: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True, default={}
    )
    ...
    orthanc_instance_id: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, index=True
    )
    orthanc_series_id: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, index=True
    )
    orthanc_study_id: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, index=True
    )
    dicom_metadata: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True, default={}
    )
    ...
    parent_document: Mapped[Optional["DocumentStorage"]] = relationship(
        "DocumentStorage",
        foreign_keys=[parent_document_id],
        remote_side="DocumentStorage.id",
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "auto_classification_confidence IS NULL OR (auto_classification_confidence BETWEEN 0 AND 100)",
...
class DocumentAccessAudit(BaseModel):
    ...
    document_id: Mapped[uuid.UUID] = mapped_column(
        UUIDType(), ForeignKey("document_storage.id"), nullable=False, index=True
    )
    user_id: Mapped[uuid.UUID] = mapped_column(
        UUIDType(), ForeignKey("users.id"), nullable=False, index=True
    )
    action: Mapped[DocumentAction] = mapped_column(
        Enum(DocumentAction), nullable=False, index=True
    )
...
class DocumentShare(BaseModel):
    ...
    permissions: Mapped[dict] = mapped_column(
        JSON, nullable=False, default={"view": True}
    )
...
class APICredentials(BaseModel):
    ...
    # Access control
    allowed_endpoints: Mapped[Optional[List[str]]] = mapped_column(
        ArrayType(String), nullable=True
    )
    rate_limit: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
...
class APIRequest(BaseModel):
    ...
    status: Mapped[str] = mapped_column(
        Enum(RequestStatus), default=RequestStatus.COMPLETED
    )
    ...
    if engine is None:
        ...
        try:
            ...
            if settings.ENVIRONMENT == "production":
                ...
                ssl_config["ssl"] = True
                logger.info(
                    "Production mode: SSL required with certificate verification"
                )
            else:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.info(
                            "SSL connection successful - using SSL preference mode"
                        )
                        ...
                        ssl_config["ssl"] = False
                        logger.warning(
                            "Fallback: Using non-SSL connection for development"
                        )
                    finally:
                        if test_conn:
                            try:
                    ...
                    logger.warning(
                        "asyncpg not available for SSL testing - using fallback"
                    )
                    ssl_config["ssl"] = False
                ...
                connect_args={
                    **connect_args,
                    ...
                    "ssl": (
                        ...
                        else ssl_config.get("ssl", "prefer")
                    ),
                },
            )
        finally:
...
def _safe_engine_cleanup(engine):
    ...
    try:
        ...
        try:
            ...
            if loop.is_closed():
                logger.warning(
                    "Event loop closed during engine cleanup - forcing synchronous cleanup"
                )
                return
        except RuntimeError:
            # No event loop available - safe to skip async cleanup
            logger.info(
                "No event loop during engine cleanup - skipping async operations"
            )
            return
...
class DatabaseSessionManager:
        ...
        try:
            ...
            try:
                ...
                if loop.is_closed():
                    ...
                    logger.error(
                        "Event loop is closed during session creation - cannot create database session",
                        session_id=self.session_id,
                    )
                    raise RuntimeError(
                        "Event loop is closed - database session creation failed"
                    )
            except RuntimeError as e:
                if "no running event loop" in str(e).lower():
                    loop_available = False
                    logger.error(
                        "No running event loop during session creation - cannot create database session",
                        session_id=self.session_id,
                    )
                    raise RuntimeError(
                        "No event loop - database session creation failed"
                    )
                else:
                    raise
            ...
            logger.debug(
                "Skipping connection test for enterprise healthcare session stability",
                session_id=self.session_id,
            )
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            ...
            try:
                ...
                if loop.is_closed():
                    logger.warning(
                        "Event loop closed during session cleanup, skipping async operations",
                        session_id=self.session_id,
                    )
                    self.session = None
                    return
                ...
                logger.warning(
                    "No event loop during session cleanup", session_id=self.session_id
                )
                self.session = None
                return
            ...
            if cleanup_tasks:
                try:
                    ...
                    logger.warning(
                        "Session transaction cleanup timed out",
                        session_id=self.session_id,
                    )
                except Exception as cleanup_error:
                    logger.error(
                        "Error during session transaction cleanup",
                        session_id=self.session_id,
            ...
            logger.error(
                "Unexpected error during session transaction cleanup",
                session_id=self.session_id,
            ...
            logger.debug(
                "Database session cleanup completed",
                session_id=self.session_id,
    ...
    async def _safe_rollback(self):
        ...
        try:
            ...
            logger.debug(
                "Database session rolled back successfully",
                session_id=self.session_id,
    ...
    async def _safe_commit_or_rollback(self):
        ...
        try:
            ...
            if self.session.dirty or self.session.new or self.session.deleted:
                ...
                logger.debug(
                    "Database session clean - no commit needed",
                    session_id=self.session_id,
                    event_type="SESSION_CLEAN",
                )
        except Exception as commit_error:
            logger.warning(
                "Failed to commit session, attempting rollback",
                session_id=self.session_id,
            ...
            try:
                await self.session.rollback()
                logger.debug(
                    "Session rolled back after failed commit",
                    session_id=self.session_id,
                )
            except Exception as rollback_error:
                logger.error(
    ...
    async def _safe_session_close(self):
        ...
        try:
            ...
            try:
                ...
                if loop.is_closed():
                    logger.warning(
                        "Event loop closed during session close, forcing cleanup",
                        session_id=self.session_id,
                    )
                ...
                logger.warning(
                    "No event loop during session close, forcing cleanup",
                    session_id=self.session_id,
                )
                self.session = None
                return
            ...
            await asyncio.wait_for(
                self.session.close(), timeout=2.0  # Reduced timeout for faster cleanup
            )
            logger.debug("Session closed successfully", session_id=self.session_id)
        except asyncio.TimeoutError:
            logger.warning("Session close timed out", session_id=self.session_id)
            # Force session reference removal
            self.session = None
        except RuntimeError as runtime_error:
            if "Event loop is closed" in str(runtime_error):
                logger.warning(
                    "Event loop closed during session cleanup, skipping async close",
                    session_id=self.session_id,
                )
                self.session = None
            else:
                logger.error(
                    "Runtime error during session close",
                    session_id=self.session_id,
    ...
    async def __aenter__(self) -> AsyncSession:
        ...
        try:
            ...
            if loop.is_closed():
                ...
                raise RuntimeError(
                    "Event loop is closed - cannot create healthcare session"
                )
        except RuntimeError as e:
            if "no running event loop" in str(e).lower():
        ...
        try:
            ...
            self.transaction_id = transaction_audit_manager.start_transaction(
                session_id=self.session_id, context="healthcare_database_session"
            )
                ...
                if exc_type:
                    ...
                    logger.warning(
                        ...
                        extra={
                            ...
                            "session_duration_ms": session_duration.total_seconds()
                            * 1000,
                    ...
                    if self.transaction_id:
                        transaction_committed = (
                            transaction_audit_manager.commit_transaction(
                                self.transaction_id
                            )
                        )
...
async def get_db() -> AsyncGenerator[AsyncSession, None]:
    ...
    try:
        ...
        if loop.is_closed():
            logger.error(
                "Event loop is closed during get_db call - cannot create session"
            )
            raise RuntimeError("Event loop is closed - database session unavailable")
    except RuntimeError as e:
        if "no running event loop" in str(e).lower():
            logger.error(
                "No running event loop during get_db call - cannot create session"
            )
...
class TransactionAuditManager:
    ...
    def validate_access_control(
        self, transaction_id: str, user_context: str = "system"
    ) -> bool:
        ...
        if user_context in ["system", "test", "admin"] or transaction["context"] in [
            "FHIR_API_TEST_USER_CREATION"
        ]:
    ...
    def _validate_phi_operation(self, table_name: str, operation: str) -> bool:
        ...
        if table_name.lower() in phi_tables:
            ...
            if (
                operation in ["INSERT", "SELECT", "UPDATE", "DELETE"]
                and table_name == "users"
            ):
                return True  # User operations are controlled by RBAC
            elif (
                operation in ["INSERT", "SELECT", "UPDATE"]
                and "patient" in table_name.lower()
            ):
                return True  # Patient data operations
            elif operation in ["INSERT", "SELECT", "UPDATE"] and table_name in [
                "roles",
                "immunizations",
                "clinical_documents",
            ]:
        ...
        for operation in transaction["operations"]:
            if not operation["compliance_validated"]:
                compliance_issues.append(
                    f"Non-compliant operation on {operation['table']}"
                )
    ...
    def commit_transaction(self, transaction_id: str) -> bool:
        ...
        if not self.validate_transaction_compliance(transaction_id):
            self.rollback_transaction(
                transaction_id, reason="compliance_validation_failed"
            )
            return False
...
async def audit_change(
    ...
    if not transaction_id or transaction_id == "unknown":
        ...
        if (
            table_name.lower() in phi_tables
            or "patient" in table_name.lower()
            or "fhir" in table_name.lower()
        ):
            ...
            logger.debug(
                "Auto-created transaction for PHI audit compliance",
                extra={
                    "transaction_id": transaction_id,
                    "table_name": table_name,
                    "operation": operation,
    ...
    if (
        auto_created_transaction
        or transaction_id in transaction_audit_manager.active_transactions
    ):
        ...
        logger.info(
            "Database operation audited without PHI context",
            extra={
                "table_name": table_name,
                "operation": operation,
                "user_id": user_id,
    ...
    if auto_created_transaction:
        try:
            ...
            logger.debug(
                "Auto-created transaction completed successfully",
                extra={"transaction_id": transaction_id, "table_name": table_name},
            )
        except Exception as e:
            logger.error(
                "Failed to complete auto-created transaction",
...
class EnterpriseConnectionManager:
    ...
    async def shutdown_all_connections(self):
        ...
        if self._active_connections:
            logger.info(
                f"Shutting down {len(self._active_connections)} active connections"
            )
            ...
            if self._active_connections:
                logger.warning(
                    f"Force closing {len(self._active_connections)} remaining connections"
                )
...
class ManagedConnection:
    ...
    def __init__(
        self, session, connection_id: str, manager: EnterpriseConnectionManager
    ):
        ...
        try:
            ...
            if exc_type:
                try:
                    ...
                    logger.warning(
                        "Session rollback timed out", connection_id=self.connection_id
                    )
                except Exception as e:
                    logger.warning(
                        f"Error during rollback: {e}", connection_id=self.connection_id
                    )
            ...
            try:
                ...
                logger.warning(
                    "Session close timed out", connection_id=self.connection_id
                )
            except Exception as e:
                logger.warning(
                    f"Error closing session: {e}", connection_id=self.connection_id
                )
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            is_concurrent_error = (
                ...
                or "session is already flushing" in error_msg
                or "connection is already provisioning" in error_msg
                or "this session is provisioning a new connection" in error_msg
                or "asyncpg" in error_msg
            )
...
async def execute_with_retry(
    session: AsyncSession, operation_func, max_retries: int = 3, *args, **kwargs
):
    """
...
class Appointment(BaseModel, SoftDeleteMixin):
    ...
    patient_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUIDType(), ForeignKey("patients.id"), nullable=True, index=True
    )
    ...
    service_type: Mapped[Optional[List[str]]] = mapped_column(
        ArrayType(String), nullable=True
    )
    ...
    participant_instructions_encrypted: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    )
...
class CarePlan(BaseModel, SoftDeleteMixin):
    ...
    # Care plan details
    category: Mapped[Optional[List[str]]] = mapped_column(
        ArrayType(String), nullable=True
    )
...
class Procedure(BaseModel, SoftDeleteMixin):
    ...
    # Procedure timing
    performed_datetime: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    performed_period_start: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    performed_period_end: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
