...
async def test_engine(test_settings: Settings):
    ...
    if (
        "postgresql" in test_settings.DATABASE_URL
        or "asyncpg" in test_settings.DATABASE_URL
    ):
        connect_args = {
            "server_settings": {
                ...
                try:
                    ...
                    if not loop.is_closed():
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Async cleanup timeout, forcing close: {cleanup_error}"
                            )
                        ...
                        logger.debug(
                            "Event loop closed during cleanup, skipping async operations"
                        )

                except RuntimeError:
                    # No event loop - this is expected during test teardown
                    logger.debug(
                        "No event loop available during cleanup - normal for test teardown"
                    )
...
async def test_purge_policy(db_session: AsyncSession) -> PurgePolicy:
    ...
    policy = PurgePolicy(
        resource_type="test_resource", retention_days=90, created_by="test_admin"
    )
...
async def healthcare_event_bus(test_session_factory) -> AsyncGenerator[Any, None]:
    ...
    try:
        ...
        logger.info(
            "Healthcare event bus initialized successfully for tests",
        ...
        logger.error(
            "Failed to initialize healthcare event bus for tests", error=str(e)
        )
...
async def override_get_event_bus(healthcare_event_bus):
    ...
    with patch.object(
        event_bus_module, "get_event_bus", return_value=healthcare_event_bus
    ):
...
async def healthcare_event_bus_fixture(test_session_factory, monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(
            "app.core.events.event_bus.get_event_bus", mock_get_event_bus
        )

        # Also patch in other modules that import it
        monkeypatch.setattr(
            "app.modules.healthcare_records.service.get_event_bus", mock_get_event_bus
        )
        monkeypatch.setattr(
            "app.modules.healthcare_records.fhir_bundle_processor.get_event_bus",
            mock_get_event_bus,
        )
...
async def async_test_client(
    override_settings, override_get_db
...
async def test_patient(db_session: AsyncSession, sample_patient_data: Dict[str, Any]):
    ...
    patient = Patient(
        ...
        first_name_encrypted=sample_patient_data[
            "first_name"
        ],  # Will be encrypted in practice
        last_name_encrypted=sample_patient_data[
            "last_name"
        ],  # Will be encrypted in practice
