...
class TestClinicalDecisionSupportEngine:
    ...
    @pytest.mark.asyncio
    async def test_evaluate_patient_diabetes_rule(
        self, cds_engine, sample_patient_data
    ):
        ...
        diabetes_alerts = [
            a
            for a in alerts
            if "diabetes" in a.title.lower() or "hba1c" in a.message.lower()
        ]
        ...
        assert alert.alert_type in [
            DecisionSupportType.QUALITY_MEASURE,
            DecisionSupportType.REMINDER,
        ]
        assert "hba1c" in alert.message.lower()

    @pytest.mark.asyncio
    async def test_evaluate_patient_hypertension_rule(
        self, cds_engine, sample_patient_data
    ):
        ...
        htn_alerts = [
            a
            for a in alerts
            if "hypertension" in a.title.lower()
            or "blood pressure" in a.message.lower()
        ]
    ...
    async def test_acknowledge_alert(self, cds_engine, sample_patient_data):
        ...
        if alerts:
            ...
            patient_alerts = await cds_engine.get_patient_alerts(
...
class TestClinicalQualityMeasures:
    ...
    async def test_calculate_diabetes_cqm(self, cds_engine, sample_diabetic_population):
        ...
        results = await cds_engine.calculate_quality_measures(
            sample_diabetic_population
        )
        ...
        assert (
            diabetes_cqm_results["denominator_count"] == 4
        )  # All 4 patients have diabetes
...
class TestClinicalAnalyticsEngine:
    ...
    @pytest.mark.asyncio
    async def test_generate_population_health_report(
        self, clinical_analytics, sample_diabetic_population
    ):
        """Test population health report generation"""

        report = await clinical_analytics.generate_population_health_report(
            sample_diabetic_population
        )
        ...
        assert all(
            level in risk_levels for level in ["low", "moderate", "high", "critical"]
        )
...
class TestClinicalDecisionSupportIntegration:
    ...
    async def test_complete_clinical_workflow(self, cds_engine, clinical_analytics):
        ...
        high_priority_alerts = [
            a
            for a in alerts
            if a.severity in [ClinicalRiskLevel.HIGH, ClinicalRiskLevel.CRITICAL]
        ]
        ...
        for cqm_id, result in cqm_results.items():
            if (
                "diabetes" in result.get("cqm_name", "").lower()
                and "poor control" in result.get("cqm_name", "").lower()
            ):
                diabetes_cqm = result
                break
        ...
        if diabetes_cqm:
            assert (
                diabetes_cqm["numerator_count"] == 1
            )  # Patient has poor control (HbA1c > 9%)

        # Step 3: Generate population health analytics
        population_report = await clinical_analytics.generate_population_health_report(
            population
        )
    ...
    @pytest.mark.asyncio
    async def test_multi_patient_population_analysis(
        self, cds_engine, clinical_analytics
    ):
