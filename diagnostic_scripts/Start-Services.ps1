        ...
        foreach ($container in $requiredContainers) {
            try {
                ...
                if ($containerStatus -and $containerStatus -match "Up") {
                    $healthyContainers++
                    if ($Verbose) {
                        Write-ServiceLog "✅ $container is running" "DEBUG"
                    }
                } else {
                    if ($Verbose) {
                        Write-ServiceLog "⏳ $container not ready yet" "DEBUG"
                    }
                }
            }
            catch {
                if ($Verbose) {
                    Write-ServiceLog "❌ Could not check $container status" "DEBUG"
                }
            }
        }
    ...
    foreach ($endpoint in $endpoints) {
        try {
            if ($endpoint -match "localhost:8000") {
                ...
                if ($response.StatusCode -eq 200) {
                    Write-ServiceLog "✅ $endpoint - API Ready" "SUCCESS"
                } else {
                    Write-ServiceLog "⚠️ $endpoint - Status: $($response.StatusCode)" "WARN"
                }
            } else {
                ...
                if ($tcpTest.TcpTestSucceeded) {
                    Write-ServiceLog "✅ Port $portNumber - Service Running" "SUCCESS"
                } else {
                    Write-ServiceLog "❌ Port $portNumber - Not Accessible" "ERROR"
                }
            }
        }
        catch {
            Write-ServiceLog "❌ $endpoint - Not Accessible" "ERROR"
        }
    }
}
...
function Write-StartupSummary {
    ...
    if ($Success) {
        ...
        Write-ServiceLog "  1. Run API tests: .\diagnostic_scripts\Test-ApiEndpoints.ps1" "INFO"
        ...
        Write-ServiceLog "  2. Run: .\diagnostic_scripts\Check-SystemHealth.ps1" "INFO"
        Write-ServiceLog "  3. Check logs: docker compose logs" "INFO"
        Write-ServiceLog "  4. Try clean restart: .\diagnostic_scripts\Start-Services.ps1 -CleanStart" "INFO"
    }
}
