# API Endpoints Diagnostic Script for IRIS API (Fixed for Windows)
...
function Invoke-ApiRequest {
    ...
    Write-TestLog "REQUEST: $Method $Endpoint" "DEBUG"
...
function Test-Authentication {
    ...
    $result = Invoke-ApiRequest -Method "POST" -Endpoint "/api/v1/auth/login" -Body $authData -ContentType "application/json"
...
function Test-GetPatient {
    if (-not $script:TestPatientId) {
        Write-TestLog "No patient ID available for get test" "WARN"
        return $false
    }
...
function Test-UpdatePatient {
    if (-not $script:TestPatientId) {
        Write-TestLog "No patient ID available for update test" "WARN"
        return $false
    }
    
    Write-TestLog "Testing Update Patient: $script:TestPatientId" "INFO"
...
function Test-ErrorHandling {
    Write-TestLog "Testing Error Handling (Non-existent Patient)" "INFO"
...
function Test-AllHealthEndpoints {
    ...
    foreach ($endpoint in $healthEndpoints) {
        ...
        if ($healthResults[$endpoint]) {
            Write-TestLog "OK: $endpoint" "PASS"
        } else {
            Write-TestLog "FAIL: $endpoint ($($result.StatusCode))" "FAIL"
        }
    }
...
function Write-ComprehensiveAnalysis {
    ...
    if ($script:FailedTests -eq 0) {
        ...
        Write-TestLog "GOOD - Some issues need fixing" "WARN"
...
if ($finalSuccessRate -eq 100) {
    ...
    Write-TestLog "MOSTLY READY - Minor fixes needed" "WARN"
    exit 1
} else {
